import { getMembers, getTeams } from "../data/teams.js";
...
const teamsTemplate = (isUser, allTeams) => html`
    <section id="browse">
        ...
        ${allTeams.map((team) => articleTemplate(team))};

    </section>
`;

async function articleTemplate(team) {
    const members = await getMembers(team._id);

    return articleTemplate = (team) => html`
